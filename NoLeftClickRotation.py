...
def _sync_camera():
    ...
    if not _edit_mode():
        _camera_lock(False)
        return
    if _right_down or _key(VK_RBUTTON):
        _camera_lock(False)
...
def _toggle(sender, value):
    del sender
    global _enabled, _right_down, _right_relock_at
    _enabled = bool(value)
    if _hwnd:
        _reset(_hwnd)
    if not _enabled:
...
def _setup_ui():
    ...
    zbc.add_switch(
        ...
        _toggle, initially_disabled=False, width=1.0,
    )
    zbc.add_slider(
        ...
        _delay_changed, initially_disabled=False, width=1.0,
    )
