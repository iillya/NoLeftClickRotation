 ...
 *
 * Loaded by the official ZBrush .zsc plugin through [FileExecute].
 * It subclasses the ZBrush main window. Left clicks always reach ZBrush. When
 * a press lands on blank canvas (Edit mode, pixol material index 0) and the
 * OS cursor is a ZBrush canvas cursor (custom/hidden, not a standard system
 * cursor), the DLL immediately sends a matching button-up so the rotation
 * drag is cancelled before it can start. UI controls (sliders, buttons,
 * custom canvas UI) use standard system cursors and are left untouched. If
 * the cursor later crosses onto the mesh, a fresh button-down is sent so the
 * brush stroke starts at the entry point.
 */
                                     ...
                                     UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
    ...
    if (g_state == ST_ARMED) {
        ...
        if (!ui_cursor && ClientToScreen(hWnd, &pt) && query_python(pt.x, pt.y, 1)) {
            /*
                                    ...
                                    char *output_memory)
{
    ...
    if (self != NULL) {
        GetModuleFileNameW(self, g_scriptPath, MAX_PATH);
        wchar_t *slash = wcsrchr(g_scriptPath, L'\\');
        if (slash != NULL) {
            slash[1] = L'\0';
...
__declspec(dllexport) BOOL WINAPI Uninstall(void)
{
    ...
    if (g_py.g_query_fn != NULL && g_py.py_dec_ref != NULL) {
        void *gil = g_py.py_gil_ensure();
