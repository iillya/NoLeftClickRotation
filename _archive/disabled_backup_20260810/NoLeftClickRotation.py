...
工作原理：
经实测确认，ZBrush 2026 的鼠标拖拽每帧都走控制器链：
  0x5E4D90 -> 0x5E53A3 -> 0x5F0FC0 -> [控制器+0x818] 对象 vtable+0x118
  = 0x1412D88A0（每帧动作处理器）。旋转（左键/右键、空白画布）和雕刻
  （左键、模型上）都经过 0x5F0FC0。0x5F0FC0 内部 0x5F1310 处
  `or [控制器+0x74c], 0x400100` 会在拖拽时置位拖拽状态标志 bit22
  （0x400000，导航标志）：空白画布导航（旋转/平移）时置位，模型雕刻
  笔划时不置位。
插件在 0x5F1310 打一个 10 字节原生存根：
    左键按住（且未按 Ctrl）-> 该 or 只写 0x100（保留 bit8），不置位
                              bit22，让 ZBrush 认为本次拖拽始终是"雕刻"，
                              从而不进入导航（视图不再旋转/平移）；
    否则                   -> 原样写 0x400100，右键、Ctrl+左键遮罩、
                              UI 均不受影响。
因此无需任何画布/UI 光标判定，也不会干扰画笔运动与界面操作。钩子全部位于
...
# 0x5F0FC0 每帧动作函数内 0x5F1310 处（拖拽导航标志置位点，17 字节窗口）：
#   81 8B 4C 07 00 00 00 01 40 00   or dword [rbx+0x74c], 0x400100
#   48 8D BB B8 07 00 00            lea rdi, [rbx+0x7b8]
# 该指令把拖拽状态 [控制器+0x74c] 的 bit8(0x100) 与 bit22(0x400000)
# 同时置位；bit22 是导航标志（旋转/平移），雕刻笔划不置位。
# 左键按下时改为只写 0x100，不置位 bit22 -> 不进入导航。
NAV_HOOKS = [
    {"name": "ROT", "rva": 0x5F1310,
     "orig": bytes.fromhex("818b4c07000000014000488dbbb8070000"), "plen": 17,
     "special": True},
]
...
def _build_branch_stub(page: int) -> bytes:
    """0x5F1310 分支钩子存根（0x70 字节，布局见函数体注释）。

    标志位（stub+0x60）== 1（左键按住且未按 Ctrl）：or 只写 0x100，
    不置位 bit22(0x400000)，让 ZBrush 认为本次拖拽始终是"雕刻"而不是
    "导航"，从而不触发视图旋转/平移。
    其余情况：原样写 0x400100，照常执行。
    注意：0x5F0FC0 内部用 r11 做帧基址，跳回必须用 rax，不能碰 r11。
    """
    st = bytearray()
    # 0x00: cmp byte [rip+0x59], 0          ; 标志在 stub+0x60
    st += b"\x80\x3D" + struct.pack("<i", 0x60 - 7) + b"\x00"
    # 0x07: je +0x23 -> 0x2C（ELSE：原逻辑）
    st += b"\x74\x23"
    # 0x09: inc dword [rip+0x5D]            ; 计数器在 stub+0x6C
    st += b"\xFF\x05" + struct.pack("<i", 0x6C - 0x0F)
    # 0x0F: or dword [rbx+0x74c], 0x100     ; 左键：只置 bit8，不置 bit22
    st += b"\x81\x8B\x4C\x07\x00\x00\x00\x01\x00\x00"
    # 0x19: lea rdi, [rbx+0x7b8]            ; 复刻原指令
    st += bytes.fromhex("488dbbb8070000")
    # 0x20: mov rax, 0x1405F1321; jmp rax   ; 跳回续点（用 rax，不能碰 r11）
    st += b"\x48\xB8" + struct.pack("<Q", 0x1405F1321) + b"\xFF\xE0"
    # 0x2D: ELSE: or dword [rbx+0x74c], 0x400100   ; 原逻辑
    st += b"\x81\x8B\x4C\x07\x00\x00\x00\x01\x40\x00"
    # 0x37: lea rdi, [rbx+0x7b8]
    st += bytes.fromhex("488dbbb8070000")
    # 0x3E: mov rax, 0x1405F1321; jmp rax
    st += b"\x48\xB8" + struct.pack("<Q", 0x1405F1321) + b"\xFF\xE0"
