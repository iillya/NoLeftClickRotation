# -*- coding: utf-8 -*-
"""Hook the 0x808 write (0x1A91521). On LMB (no RMB) force 0x808=0 -> no nav."""
...
HOOK = {
    "name": "wr_808",
    "rva": 0x1A91521,
    "orig": bytes.fromhex("48898308080000488b8300080000"),
    "cont": 0x1401A9152F,
}
...
def _install() -> bool:
    ...
    page = int(page)
    stb = bytearray()
    # 0x00: mov qword [rbx+0x808], rax (original write)
    stb += bytes.fromhex("48898308080000")
    # 0x07: mov [rip+0x52], rbx -> +0x60
    stb += b"\x48\x89\x1D" + struct.pack("<i", 0x60 - 0x0E)
    # 0x0E: cmp byte [rip+0x5B], 0 (flag @ +0x70)
    stb += b"\x80\x3D" + struct.pack("<i", 0x70 - 0x15) + b"\x00"
    # 0x15: je +11 -> 0x22 (flag=0 skip overwrite)
    stb += b"\x74\x0B"
    # 0x17: mov qword [rbx+0x808], 0
    stb += b"\x48\xC7\x83\x08\x08\x00\x00\x00\x00\x00\x00"
    # 0x22: mov rax, [rbx+0x800]
    stb += bytes.fromhex("488b8300080000")
    # 0x29: inc qword [rip+0x48] -> +0x78
    stb += b"\x48\xFF\x05" + struct.pack("<i", 0x78 - 0x30)
    # 0x30: mov r11, cont ; jmp r11
...
def _snap():
    obj = ctypes.c_uint64.from_address(_stub + 0x60).value
...
def _tick() -> None:
    ...
    try:
        ...
        cur = ctypes.c_uint8.from_address(_stub + 0x70).value
        if cur != flag:
        ...
        if not _recording:
            return
        obj, v808, v74c, cnt = _snap()
        now = time.time()
        if now - _last_report >= 0.15:
            _last_report = now
            _dlog("t=%.1f down=%d rdown=%d flag=%d obj=%#x 808=%s 74c=%s n=%d"
                  % (now % 1000, left, right, flag, obj, v808,
